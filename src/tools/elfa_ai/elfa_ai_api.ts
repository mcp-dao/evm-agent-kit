...
function createAxiosInstance(apiKey: string | undefined): AxiosInstance {
  if (!apiKey) {
    throw new Error("ELFA_AI_API_KEY is not configured in EvmAgentKit config.");
  }
  return axios.create({
...
export async function pingElfaAiApi(agent: EvmAgentKit): Promise<any> {
...
export async function getElfaAiApiKeyStatus(agent: EvmAgentKit): Promise<any> {
  const apiKey = agent.config?.ELFA_AI_API_KEY;
  const axiosInstance = createAxiosInstance(apiKey);
