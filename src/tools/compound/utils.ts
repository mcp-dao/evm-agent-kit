...
import { COMET_ABI, PRICE_FEED_ABI } from "./constants";
...
export const getHealthRatio = async (
  ...
  return borrowValue.eq(0)
    ? new Decimal(Infinity)
    : totalAdjustedCollateral.div(borrowValue);
};
...
export const getHealthRatioAfterBorrow = async (
  ...
  return newBorrowValue.eq(0)
    ? new Decimal(Infinity)
    : totalAdjustedCollateral.div(newBorrowValue);
};
...
const getBorrowDetails = async (
  ...
  const humanBorrowAmount = new Decimal(
    formatUnits(borrowAmountRaw, baseDecimals),
  );
  const price = new Decimal(basePriceRaw).div(new Decimal(10).pow(8));

  return {
    tokenSymbol: baseTokenSymbol,
    borrowAmount: humanBorrowAmount,
    price,
  };
};
...
> => {
  ...
  for (let i = 0; i < numAssets; i++) {
    ...
    if (collateralBalance > 0n) {
      ...
      const humanSupplyAmount = new Decimal(
        formatUnits(collateralBalance, decimals),
      );
      const price = new Decimal(priceRaw).div(new Decimal(10).pow(8));
      const collateralFactor = new Decimal(
        assetInfo.borrowCollateralFactor.toString(),
...
export const getCometAddress = (network: Network): Address => {
  ...
  if (network.networkId === "ethereum-mainnet") {
    return COMET_ADDRESSES["ethereum-mainnet"];
  }
  // else if (network.networkId === "ethereum-sepolia") {
  //   return COMET_ADDRESSES["ethereum-sepolia"];
  // }
...
export const getAssetAddress = (network: Network, assetId: string): Address => {
  ...
  if (network.networkId === "ethereum-mainnet") {
    const address = ASSET_ADDRESSES["ethereum-mainnet"][normalizedAssetId];
    if (!address) {
      throw new Error(`Asset ${assetId} not supported on Ethereum Mainnet`);
    }
    return address;
  }
  // else if (network.networkId === "ethereum-sepolia") {
  //   const address = ASSET_ADDRESSES["ethereum-sepolia"][normalizedAssetId];
  //   if (!address) {
  //     throw new Error(`Asset ${assetId} not supported on Ethereum Sepolia`);
  //   }
  //   return address;
  // }
