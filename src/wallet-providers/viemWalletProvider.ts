...
export class ViemWalletProvider extends EvmWalletProvider {
  ...
  constructor(
    ...
    this.#gasLimitMultiplier = Math.max(
      gasConfig?.gasLimitMultiplier ?? 1.2,
      1,
    );
    this.#feePerGasMultiplier = Math.max(
      gasConfig?.feePerGasMultiplier ?? 1,
      1,
    );
  }
  ...
  async sendTransaction(
    ...
    const maxFeePerGas = applyGasMultiplier(
  ...
  >(
    ...
    return this.#walletClient.writeContract<abi, functionName, args>(params);
  }
   ...
   * @returns The transaction hash.
   */
  async nativeTransfer(to: Address, value: string): Promise<`0x${string}`> {
